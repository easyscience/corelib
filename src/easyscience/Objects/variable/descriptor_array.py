...
class DescriptorArray(DescriptorBase):
    ...
    def __init__(
        ...
        param dims: List of dimensions to pass to scipp. Will be autogenerated if not supplied.
        ...
        if dims is None:
            ...
            dims = ['dim'+str(i) for i in range(len(value.shape))]
        if not len(dims) == len(value.shape):
            raise ValueError(f"Length of dims ({dims=}) does not match length of value {value=}.")
        self._dims = dims

        try:
            self._array = sc.array(dims=dims, values=value, unit=unit, variances=variance)
    ...
    def dims(self) -> list:
        """
        Get the dims used for the underlying scipp array.
    ...
    @dims.setter
    def dims(self, dims: Union[list, np.ndarray]) -> None:
        """
        Set the dims of self. Ensures that the input has a shape compatible with self.full_value.
    ...
    def __setitem__(self, a, b: Union[numbers.Number, list, DescriptorNumber, DescriptorArray]):
        ...
        """
        raise AttributeError(
            f'{self.__class__.__name__} cannot be edited via slicing. Edit the underlyinf scipp\
